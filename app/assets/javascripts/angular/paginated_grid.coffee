...
.directive 'mflGrid', () ->
	...
	link: (scope, elem, attrs) ->
		...
		page_context = (current_page, last_page) ->
			(i for i in [(Math.max(1, current_page - 10))..(Math.min(parseInt(current_page) + 10, last_page))])
		...
		scope.previous_page = () ->
			scope.change_page(parseInt(scope.current_page) - 1) unless scope.current_page == 1

		scope.next_page = () ->
			scope.change_page(parseInt(scope.current_page) + 1) unless scope.current_page == scope.last_page
